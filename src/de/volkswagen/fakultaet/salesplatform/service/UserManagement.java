...
public class UserManagement {
	...
	public static Customer registerCustomer(User[] users) {
		...
		String surname = InputManagement.getSurnameInput();
		String street = InputManagement.getStreetInput();
		String houseNumber = InputManagement.getHouseNumberInput();
		String zipCode = InputManagement.getZipCodeInput();
		String city = InputManagement.getCityInput();
	...
	*/
	...
	public static void changeData(User user) {
		int choice;
		while (true) {
			choice = InputManagement.getUserFieldToChange(user);

			if (choice == 0) {
				break;
			} else if (choice == 1) {
				user.setFirstName(InputManagement.getFirstNameInput());
			} else if (choice == 2) {
				user.setSurname(InputManagement.getSurnameInput());
			} else if (choice == 3) {
				user.setStreet(InputManagement.getStreetInput());
			} else if (choice == 4) {
				user.setHouseNumber(InputManagement.getHouseNumberInput());
			} else if (choice == 5) {
				user.setZipCode(InputManagement.getZipCodeInput());
			} else if (choice == 6) {
				user.setCity(InputManagement.getCityInput());
			} else if (choice == 7 && user instanceof Administrator) {
				Administrator admin = (Administrator) user;
				admin.setPassword(InputManagement.getPasswordInput());
			} else {
